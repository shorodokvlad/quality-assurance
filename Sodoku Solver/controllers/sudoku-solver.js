...
class SudokuSolver {
  ...
  checkRegionPlacement(puzzleString, row, column, value) {
    ...
    for(let i = 0; i < 3; i++){
      for(let j = 0; j < 3; j++) {
        if(i + startRow === row - 1 && j + startCol === column - 1) continue
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        if (grid[r][c] !== 0) {
          ...
          for (let i = 0; i < 3; i++) {
            for (let j = 0; j < 3; j++) {
              let currRow = i + startRow;
              let currCol = j + startCol;
              if ((currRow !== r || currCol !== c) && grid[currRow][currCol] === val) {
                return false;
              }
            }
          }
  ...
  solve(puzzleString) {
    ...
    if (!this.checkInitialGridValid(grid)) return false
